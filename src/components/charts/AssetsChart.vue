...
const chartKey = computed(() => `${isDark.value}-${props.points.length}-${props.yearTicks}`)

// Intervalos curtos: primeiro tick = ano (ex.: "2026") e mudanças de mês = nome do mês.
// Intervalos longos (yearTicks): só mudanças de ano.
const tickLabels = computed<(string | null)[]>(() => {
  ...
  for (let i = 0; i < n; i++) {
    ...
    } else if (dateStr.substring(0, 7) !== prev.substring(0, 7)) {
    ...
    plugins: {
      ...
      tooltip: {
        ...
        callbacks: {
          ...
          label: (ctx: { raw: unknown }) => fmt(Number(ctx.raw)),
        },
      },
    },
    scales: {
